...
export function draw(space: Space, blended: BlendedSpaceIndex[]) {
  ...
  const [boxWidth, boxHeight, boxDepth] = space.dimensions;
  ...
  for (const { source, target } of blended) {
    const sphereGeometry = new SphereGeometry(3, 32, 16);
    const redMesh = new MeshBasicMaterial({
      color: 0xff0000,
      transparent: true,
      opacity: 0.75,
    });
...
export async function drawToFile(
  space: Space,
  blended: BlendedSpaceIndex[],
  file: string
) {
