...
export function setOptions(element: HTMLElement, options: unknown) {
  ...
  function set(key: string | symbol, value: unknown) {
    ...
    if (isArrayLike(value)) {
      ...
      const list: DOMTokenList = isDOMTokenList(elementValue)
        ? elementValue
        : undefined;
      const array =
    ...
    } else if (isObject(value) && isObject(elementValue)) {
      const keys = Object.keys(value);
      const existingKeys = Object.keys(elementValue);
      const removeKeys = existingKeys.filter((key) => !keys.includes(key));
      for (const key of removeKeys) {
        elementValue[key] = undefined;
      }
      for (const key of keys) {
        elementValue[key] = value[key];
      }
    } else {
      element[key] = value;
    }
...
export function setAttributes(element: HTMLElement, options: unknown) {
  const attributes = Object.entries(options ?? {})
    .filter(
      ([key, value]) => typeof key === "string" && isStaticChildNode(value)
    )
  ...
  for await (const descendants of descendantsSettled(node)) {
    ...
    for (const parent of parents.keys()) {
      ...
      for (let index = 0; index < maxChildNodes; index += 1) {
        ...
        const previous = reverseNodes.find(
          (node, nodeIndex) => !!node && nodeIndex < index
        );
